...
class Union(XdrType):
    ...
    def __getattr__(self, name):
        if name in ('switch', self._union_switch_name):
            return self._union_arm_info['switch']
        elif name == self._union_arm_info['name']:
            return self
        else:
    ...
    def __setattr__(self, name, value):
        ...
        elif name in ('switch', self._union_switch_name):
            if self._union_arm_type(value) is self.__class__:
                self._union_arm_info['switch'] = value
            else:
    ...
    def __delattr__(self, name):
        if name.startswith('_'):
            ...
            self._union_arm_info['switch'] = None
        else:
            super().__delattr__(name)
    ...
    def _get_item(cls, index):
        index = cls._union_switch_type(index)
        arm_class = cls._union_arm_type.get(index)
        arm_name = cls._union_arm_name.get(index)
        ...
        arm_class._union_arm_info['switch'] = index
        arm_class._union_arm_info['name'] = arm_name
        return arm_class
    ...
    @classmethod
    def parse(cls, bstr):
        switch, bstr = cls._union_switch_type.parse(bstr)
        arm_class = cls[switch]
        data, bstr = arm_class._union_arm_info['type'].parse(bstr)
